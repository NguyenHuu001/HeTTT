...
function Login() {
    ...
    useEffect(() => {}, []);
    const getKeys = async () => {
        const dataToEncrypt = 'xin chào';
        try {
            const response = await fetchKeys().then((res) => {
                console.log(encrypt(res.package));
...
const encryptedData = publicKeyObj.encrypt(dataToEncrypt, 'RSA-OAEP');

console.log('Dữ liệu đã mã hóa:', forge.util.encode64(encryptedData));
            });
        } catch (error) {
            console.log('lỗi');
        }
